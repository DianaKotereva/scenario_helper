            ...
            try:
                ...
                if isinstance(result, ReasoningOutput):
                    ...
                    try:
                        ...
                        logger.error(f"Validation error in reasoning output: {validation_error}", exc_info=True)
                    ...
                    logger.warning(f"Reasoning agent returned unexpected type: {type(result)}")
                    state["next_step"] = NextStep.SEARCH.value
                    state["n_iteration"] = state.get("n_iteration", 0) + 1
                    return state
                ...
                logger.debug(f"Reasoning result validated: next_step={reasoning_output.next_step}, "
...
def cond_edge_reasoner(state: AgentState) -> str:
    ...
    try:
        ...
        if should_search:
            logger.debug(f"Continuing search (iteration {n_iteration}/{max_iterations})")
            return "search_node"
        else:
...
def search_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        context = state.get("context", [])
        done_queries = [list(item.keys())[0] for item in context if isinstance(item, dict) and item]
            ...
            try:
                ...
                if isinstance(result, QuestionGeneratorOutput):
                    ...
                    try:
                        ...
                        logger.error(f"Validation error in questions output: {validation_error}", exc_info=True)
                        questions = []
                    else:
                        questions = questions_output.questions
                else:
                    logger.warning(f"Questions agent returned unexpected type: {type(result)}")
                    questions = []
                ...
                try:
                    ...
                    if isinstance(result, RetrieveOutput):
                        ...
                        try:
                            ...
                            logger.error(f"Validation error in retrieve output for query '{query}': {validation_error}", exc_info=True)
                            # Fallback: используем значение из словаря, если есть
                            answer = result.get("answer", "")
                            if not answer or len(answer) < 20:
                                logger.warning(f"Invalid answer for query '{query}', skipping")
                                continue
                    else:
                        logger.warning(f"Retrieve agent returned unexpected type: {type(result)} for query: {query}")
                        continue
                    
                    if answer:
                        state["context"].append({query: answer})
                        logger.debug(f"Found validated answer for query: {query[:50]}... (length: {len(answer)})")
                    else:
                        logger.warning(f"No answer found for query: {query}")
                        
                except Exception as e:
                    logger.error(f"Error retrieving answer for query '{query}': {e}", exc_info=True)
                    continue
        else:
        ...
        try:
            ...
            if isinstance(result, FinalAnswerOutput):
                ...
                try:
                    ...
                    logger.error(f"Validation error in final answer output: {validation_error}", exc_info=True)
                    # Fallback: используем значение из словаря, если есть
            ...
            if final_answer and len(final_answer) >= 50:
                ...
                logger.info(f"Final answer generated successfully (length: {len(final_answer)})")
            else:
                logger.warning(f"Final answer too short or empty: {len(final_answer) if final_answer else 0}")
                state["final_answer"] = "Не удалось сформировать полный ответ на основе доступной информации."
