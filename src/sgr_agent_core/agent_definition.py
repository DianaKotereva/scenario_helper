...
class LLMConfig(BaseModel, extra="allow"):
    ...
    def to_openai_client_kwargs(self) -> dict[str, Any]:
        kwargs = self.model_dump(exclude={"api_key", "base_url", "proxy"})
        # OpenAI-compatible GPT-5 endpoints expect max_completion_tokens instead of max_tokens.
        base_url_l = (self.base_url or "").lower()
        if "openai" in base_url_l or "proxyapi" in base_url_l:
...
class AgentDefinition(AgentConfig):
    ...
    def agent_level_config_override_validator(cls, data):
        ...
        # Avoid recursive GlobalConfig() construction during GlobalConfig.from_yaml().
        global_config = GlobalConfig._instance
    ...
    def agent_level_tools_validator(self) -> Self:
        ...
        # Avoid recursive GlobalConfig() construction during GlobalConfig.from_yaml().
        global_config = GlobalConfig._instance
        global_tools = global_config.tools if global_config is not None else {}
        processed = []
