...
###############################################"""
        ...
        self.log.append(
            {
                ...
                "tool_call_seq": self._tool_call_seq,
        ...
        self._write_trace_file(
            trace_type="tool",
            seq=self._tool_call_seq,
            payload={
                "step_number": self._context.iteration,
                "timestamp": datetime.now().isoformat(),
                "tool_name": tool.tool_name,
                "tool_input": tool.model_dump(mode="json"),
                "tool_output": result,
            },
        )
    ...
    def _log_llm_call(
        ...
        *,
        ...
        """Log one LLM call (request/response) to in-memory log and per-call file."""
        ...
        self._write_trace_file(trace_type="llm", seq=self._llm_call_seq, payload=entry)

    def _write_trace_file(self, *, trace_type: str, seq: int, payload: dict[str, Any]) -> None:
        from sgr_agent_core.agent_config import GlobalConfig

        logs_dir = GlobalConfig().execution.logs_dir
        if not logs_dir:
            return
        trace_root = Path(logs_dir) / "llm_tool_traces" / self.id
        trace_root.mkdir(parents=True, exist_ok=True)
        safe_type = "llm" if trace_type == "llm" else "tool"
        filename = f"{seq:04d}_{safe_type}.json"
