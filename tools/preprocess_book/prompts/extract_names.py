...
1. Извлеки именованные сущности как узлы графа:
   ...
   - classification: строго один из ['{CLASS_CHARACTER}', '{CLASS_PLACE}', '{CLASS_ORG}', '{CLASS_TERM}', '{CLASS_FORCE}']. null запрещен, тебе запрещено использовать его. Ты можешь использовать ТОЛЬКО что-то из того, что тебе дано в списке выше. Учти, это обязательное поле. Если нода попадает под несколько типов, впиши один из них.
...
2. Приоритеты:
   ...
   - Ты НЕ ИМЕЕШЬ права упускать информацию, которая указана в тексте. Лучше выписать больше, чем меньше!
